...
# Verify the real launcher before using the CI-only export activity.
adb shell am start -W -n "$PKG/.HomeActivity" > "$RESULT_DIR/home-start.txt" 2>&1 || true
sleep 5
...
adb shell mkdir -p "$DEVICE_DIR" > "$RESULT_DIR/mkdir.txt" 2>&1 || true
for f in v408-results/fixtures/*.mp4; do
  name="$(basename "$f")"
  if ! adb push "$f" "$DEVICE_DIR/$name" > "$RESULT_DIR/push-${name}.txt" 2>&1; then
    echo "direct push failed for $name, trying run-as"
    tmp="/data/local/tmp/$name"
    adb push "$f" "$tmp" >> "$RESULT_DIR/push-${name}.txt" 2>&1 || PASS=0
    adb shell chmod 644 "$tmp" >/dev/null 2>&1 || true
    adb shell run-as "$PKG" mkdir -p "$DEVICE_DIR" >/dev/null 2>&1 || true
    adb shell run-as "$PKG" cp "$tmp" "$DEVICE_DIR/$name" >> "$RESULT_DIR/push-${name}.txt" 2>&1 || PASS=0
  fi
done
...
run_case() {
  ...
  adb logcat -c
  timeout 30s adb shell am start -W -S -n "$ACT" \
  ...
  adb shell cat "$device_marker" | tr -d '\r' > "$RESULT_DIR/result-${case_name}.txt"
  cat "$RESULT_DIR/result-${case_name}.txt"
  grep -q '^PASS' "$RESULT_DIR/result-${case_name}.txt" || return 1
  adb pull "$device_output" "$local_output" > "$RESULT_DIR/pull-${case_name}.txt" 2>&1 || return 1
...
if grep -E 'FATAL EXCEPTION|AndroidRuntime.*Process: com\.bianzhifeng\.jinghua' "$RESULT_DIR/logcat-final.txt"; then
